...
void main()
{
	...
	//float height = texture2D(material.displacementMap, TexCoord).r * heightScale;
	...
	//color = texture2D(material.diffuseTex, vs_texCoord + height * worldNormal.xy);

	color = vec4(normals, 1.);

	//float height = texture(material.displacementMap, scaled_uv).r;
	//vec3 normal = texture(material.normalMap, scaled_uv).rgb;
	//color = vec4(normal,1.);
	//color = vec4(height,height,height,1.);

	//vec3 normals = texture(material.normalMap, vec2(vs_texcoord.y * material.textureScale.y,vs_texcoord.x * material.textureScale.x)).rgb;
	//normals = normals * 3.f;
	//normals.x = normals.x + vs_normal.x * 4.;
	//normals.y = normals.y + vs_normal.y * 4.;
	//normals.z = normals.z + vs_normal.z * 4.;
	//normals = normalize(normals * 2.0 - 1.0);
	//color = vec4(normals* 0.1,1.f);
	...
	//color = texture(material.diffuseTex, scaled_uv) * ((vec4(ambientFinal, 1.0) +  (vec4(diffuseFinal.xyz, 1.0) + vec4(specularFinal.xyz, 1.0))) * pointLight.intensity); 
	//color = texture(material.diffuseTex, scaled_uv) * ((vec4(ambientFinal, 1.0) +  (vec4(diffuseFinal.xyz, 1.0))) * pointLight.intensity);  //vec4(specularFinal.xyz, 1.0))) * pointLight.intensity); 
	//color = vec4(specularFinal.xyz, 1.0);
	//color = vec4(color.x,color.y,color.z,material.alpha);
	//color = vec4(color.x,color.y,color.z,material.alpha);

	//vec3 roughness = texture(material.roughnessMap, vec2(vs_texcoord.y * material.textureScale.y * 8,vs_texcoord.x * material.textureScale.x * 8)).rgb;
	

	//color = vec4(roughness,1.);
